...
<div className="d-flex flex-row text-wrap bg-gray-100 badge badge-sm border-top border-bottom rounded-0 text-secondary align-items-start justify-content-center mb-2">
<span className="col-8 text-start text-secondary text-xs text-nowrap font-weight-semibold opacity-7 p-2 mb-0 ps-3 ps-sm-8">Ocena / Login</span>
...
<>
                       ...
                       <li key={opinion.id} className="list-group-item border-0 d-flex align-items-stretch px-0 mb-0">
                  <div className='d-flex flex-column text-center'>
                    <div className="avatar avatar-xl rounded-circle position-relative border border-gray-100 border-3 shadow-sm">
                    <Link type="button" className='border-0 bg-transparent p-0' href={`/profile-${opinion.user.id}-${opinion.user.nick.replace(" ", "-")}`} method="get" as="button">
                      <img src={Boolean(opinion.user.avatar) ? `storage/${opinion.user.avatar}` : `storage/images/user-${Boolean(opinion.user.sex == 'Kobieta') ? 'female' : 'male'}-icon.png`} alt="profile_image" className="w-100 h-100" />
                    </Link>
              </div>
    ...
    <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
                            ...
                            <Link type="button" className='border-0 bg-transparent wine-name-link text-wrap' href={`/profile-${opinion.user.id}-${opinion.user.nick.replace(" ", "-")}`} method="get" as="button">
                              {opinion.user.nick}
                            </Link>
                            </h4> </th>
                ...
                </> : <p className='text-center text-xs text-secondary mt-2 opacity-75'>Te wino nie posiada jeszcze żadnych opinii.</p>}
            </>
    );
}
